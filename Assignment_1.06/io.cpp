...
void io_display(dungeon_t *d)
{
  ...
  if(d->illuminated) {
    ...
    for(y = 0; y < 21; y++) {
      for(x = 0; x < 80; x++) {
	if(d->character[y][x] && (abs(y - d->pc.position[dim_y]) < 3)
	   && (abs(x - d->pc.position[dim_x]) < 3)) {
	  mvaddch(y + 1, x, d->character[y][x]->symbol);
	} else {
	  switch (known_mapxy(x, y)) {
	  case ter_wall:
	  case ter_wall_immutable:
	    mvaddch(y + 1, x, ' ');
	    break;
...
uint32_t io_teleport_mode(dungeon_t *d) {
  int move, set, check_x, check_y;
  check_x = d->pc.position[dim_x];
  check_y = d->pc.position[dim_y];
  set = 1;
  d->pc.symbol = '*';
  d->illuminated = 1;
  do {
    ...
    case 'y':
    case KEY_HOME:
      move_teleporting_pc(d, 7);
      break;
    case '8':
    case 'k':
    case KEY_UP:
      move_teleporting_pc(d, 8);
      break;
    case '9':
    case 'u':
    case KEY_PPAGE:
      move_teleporting_pc(d, 9);
      break;
    case '6':
    case 'l':
    case KEY_RIGHT:
      move_teleporting_pc(d, 6);
      break;
    case '3':
    case 'n':
    case KEY_NPAGE:
      move_teleporting_pc(d, 3);
      break;
    case '2':
    case 'j':
    case KEY_DOWN:
      move_teleporting_pc(d, 2);
      break;
    case '1':
    case 'b':
    case KEY_END:
      move_teleporting_pc(d, 1);
      break;
    case '4':
    case 'h':
    case KEY_LEFT:
      move_teleporting_pc(d, 4);
      break;
    case 'g':
      set = 0;
      break;
  ...
  if(mappair(d->pc.position) == ter_wall) {
    mappair(d->pc.position) = ter_floor_hall;
    hardnesspair(d->pc.position) = 0;
  }
  
  dijkstra_tunnel(d);

  if(d->pc.position[dim_x] == check_x && d->pc.position[dim_y] == check_y) {
    return 1;
  } else {
    return 0;
  }
