...
static int make_monsters(dungeon_t *d) {
  d->characters = malloc(sizeof(*d->characters) * ((d->num_monsters) + 1));

  for(int i = 1; i <= d->num_monsters; i++) {
    d->characters[i].priority = i+1;
    d->characters[i].stats = (uint8_t) rand_range(0,15);
    d->characters[i].speed = (uint8_t) rand_range(5,20);
    d->characters[i].turn = 0;
    d->characters[i].is_alive = 1;
    sprintf(d->characters[i].name, "%x", d->characters[i].stats);
    d->characters[i].last_known_pos[dim_x] = -1;
    d->characters[i].last_known_pos[dim_y] = -1;
  }

  int16_t the_x, the_y, the_room, skip, j, i;
  skip = j = 0;
  for(i = 1; i < (d->num_monsters)+1; i++) {    
    do {
      ...
      the_y = 0;
      the_room = 0;
      the_room = rand()%(d->num_rooms-1) +1;
      the_x = rand()%d->rooms[the_room].size[dim_x];
      the_y = rand()%d->rooms[the_room].size[dim_y];
      the_x += d->rooms[the_room].position[dim_x];
      the_y += d->rooms[the_room].position[dim_y];
      for(j = 1; j < i; j++) {
	if(d->characters[j].position[dim_x] == the_x
	   && d->characters[j].position[dim_y] == the_y) {
	  skip = 1;
	}
      }
      j=1;
    ...
    }while(mapxy(the_x,the_y) != ter_floor_room);

    
    d->characters[i].position[dim_x] = the_x;
    d->characters[i].position[dim_y] = the_y;
  }
...
void render_dungeon(dungeon_t *d){
  ...
  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
	...
	if(p[dim_y] == d->characters[i].position[dim_y]
	   && p[dim_x] == d->characters[i].position[dim_x]
	   && d->characters[i].is_alive) {
	  put_character = d->characters[i].name[0];
	  i = d->num_monsters+1;
	  is_character = 1;
	}
      }
...
int write_dungeon(dungeon_t *d, char *file)
{
  ...
  fwrite(&d->characters[0].position[dim_x], 1, 1, f);
  fwrite(&d->characters[0].position[dim_y], 1, 1, f);
...
int read_dungeon(dungeon_t *d, char *file)
{
  ...
  fread(&d->characters[0].position[dim_x], 1, 1, f);
  fread(&d->characters[0].position[dim_y], 1, 1, f);
...
void render_movement_cost_map(dungeon_t *d)
{
  ...
  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
    for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) {
      if (p[dim_x] ==  d->characters[0].position[dim_x] &&
          p[dim_y] ==  d->characters[0].position[dim_y]) {
        putchar('@');
      } else {
...
{
  d->characters[0].priority = 1;
  d->characters[0].is_alive = (uint8_t) 1;
  d->characters[0].speed = (uint8_t) 10;
  d->characters[0].stats = (uint8_t) 15;
  d->characters[0].name[0] = '@';
  d->characters[0].turn = 0;
  d->characters[0].last_known_pos[dim_x] = d->characters[0].position[dim_x];
  d->characters[0].last_known_pos[dim_y] = d->characters[0].position[dim_y];
}


static int32_t turn_cmp(const void *key, const void *with)
{
  return (((character_t *) key)->turn -
    ((character_t *) with)->turn) ?
    (((character_t *) key)->turn - ((character_t *) with)->turn)
    : (((character_t *) key)->priority -((character_t *) with)->priority);
}
...
void play_dungeon(dungeon_t *d)
{
  ...
  for(i = 0; i < d->num_monsters; i++) {
    d->characters[i + 1].stats = 7;
    sprintf(d->characters[i+1].name,"%x",d->characters[i+1].stats);
    d->characters[i + 1].speed = 20;
  }
  */
  ...
  while(d->characters[0].is_alive && d->num_monsters > 0) {
    c = heap_remove_min(&h);
    if(c->name[0] == '@') {
      usleep(25000);
      render_dungeon(d);
    }
    if(!(d->characters[0].last_known_pos[dim_x] == d->characters[0].position[dim_x]
	  && d->characters[0].last_known_pos[dim_y] == d->characters[0].position[dim_y])) {
      dijkstra(d);
      dijkstra_tunnel(d);
    }
...
{
  int a = MONSTER_NUM(c->name);
  if(c->name[0] == '@') {
    is_PC(d);
  } else if(within_vision(d,c) && !CHECK_INT(a)) {
    move_to_pc(d, c);
  } else if(CHECK_ERR(a) && rand_range(0,1)) {
    move_random(d,c);
  } else if(CHECK_TEL(a) && CHECK_INT(a)) {
    move_int_tel(d,c);
  } else if(CHECK_INT(a)) {
    move_int(d,c);
  }
...
{
  if(((abs(d->characters[0].position[dim_x] - c->position[dim_x]) < 10)
     && (abs(d->characters[0].position[dim_y] - c->position[dim_y]) < 10))
     || CHECK_TEL((int)c->stats)) {
    c->last_known_pos[dim_x] = d->characters[0].position[dim_x];
    c->last_known_pos[dim_y] = d->characters[0].position[dim_y];
    return 1;
  } else {
    return 0;
  }
...
void move_random(dungeon_t *d, character_t *c)
{
  ...
  int x_next, y_next;
  do {
  x_next = rand_range(-1, 1);
  y_next = rand_range(-1, 1);
  } while(mapxy((c->position[dim_x] + x_next), (c->position[dim_y] + y_next)) == ter_wall_immutable
	  || (mapxy(c->position[dim_x] + x_next, c->position[dim_y] + y_next) == ter_wall
	      && (!CHECK_TUN(a))));

  pair_t temp;
  temp[dim_x] = c->position[dim_x] + x_next;
  temp[dim_y] = c->position[dim_y] + y_next;
  check_if_kills(d, c, temp);


  if(mapxy(c->position[dim_x]+x_next, c->position[dim_y]+y_next) == ter_wall) {
    if(hardnessxy(c->position[dim_x]+x_next, c->position[dim_y]+y_next) < 85) {
      
      c->position[dim_x] += x_next;
      c->position[dim_y] += y_next;
      d->map[c->position[dim_y]][c->position[dim_x]] = ter_floor_hall;
      d->hardness[c->position[dim_y]][c->position[dim_x]] = 0;
    } else {
      d->hardness[c->position[dim_y]+y_next][c->position[dim_x]+x_next] -= 85;
    }
  } else {
    c->position[dim_x] += x_next;
    c->position[dim_y] += y_next;
...
void is_PC(dungeon_t *d)
{
  ...
  do {
    ...
    y_next = p[i][1];
    i++;
  }while(mapxy(d->characters[0].position[dim_x]+x_next, d->characters[0].position[dim_y]+y_next) == ter_wall_immutable);

  pair_t temp;
  temp[dim_x] = d->characters[0].position[dim_x] + x_next;
  temp[dim_y] = d->characters[0].position[dim_y] + y_next;

  check_if_kills(d, &(d->characters[0]), temp);

  if(mapxy(d->characters[0].position[dim_x]+x_next, d->characters[0].position[dim_y]+y_next) == ter_wall) {
    if(hardnessxy(d->characters[0].position[dim_x]+x_next, d->characters[0].position[dim_y]+y_next) < 85) {
      d->characters[0].position[dim_x] += x_next;
      d->characters[0].position[dim_y] += y_next;
      d->map[d->characters[0].position[dim_y]][d->characters[0].position[dim_x]] = ter_floor_hall;
      d->hardness[d->characters[0].position[dim_y]][d->characters[0].position[dim_x]] = 0;
    } else {
      d->hardness[d->characters[0].position[dim_y]+y_next][d->characters[0].position[dim_x]+x_next] -= 85;
    }
  } else {
    d->characters[0].position[dim_x] += x_next;
    d->characters[0].position[dim_y] += y_next;
...
{     
  ...
  int p[8][2] = {{-1,-1},
	       ...
	       {0,1},
	       {1,1}};
  closest_pair[dim_x] = c->position[dim_x];
  closest_pair[dim_y] = c->position[dim_y];


  //Order of my if statement:
  //1) It checks the x and y positions of closest_pair to temp and if temp is lower in both
  //categories it proceeds to the next option.
  //2) Then it check if it is tunnelling type or if the new pair is moving to a place that is
  //not a wall. 
  //3) Then it checks the hardness of temp if less than the hardness of closest_pair.
  pair_t temp;
  for(int i = 0; i < 8; i++) {
    temp[dim_x] = c->position[dim_x] + p[i][0];
    temp[dim_y] = c->position[dim_y] + p[i][1];
    if((CHECK_TUN(a) && (d->pc_tunnel[temp[dim_y]][temp[dim_x]]
			 < d->pc_tunnel[closest_pair[dim_y]][closest_pair[dim_x]]))
	|| ((mappair(temp) != ter_wall)
	    && (d->pc_distance[temp[dim_y]][temp[dim_x]]
		< d->pc_distance[closest_pair[dim_y]][closest_pair[dim_x]]))){
	 closest_pair[dim_x] = temp[dim_x];
	 closest_pair[dim_y] = temp[dim_y];
    }
  }

  check_if_kills(d, c, closest_pair);

  if((mappair(closest_pair) == ter_wall) && (CHECK_TUN(a))) {
    if(hardnesspair(closest_pair) < 85) {
      c->position[dim_x] = closest_pair[dim_x];
      c->position[dim_y] = closest_pair[dim_y];
      d->hardness[closest_pair[dim_y]][closest_pair[dim_y]] = 0;
      d->map[closest_pair[dim_y]][closest_pair[dim_x]] = ter_floor_hall;
    } else {
      d->hardness[closest_pair[dim_y]][closest_pair[dim_x]] -= 85;
    }
  } else {
    c->position[dim_x] = closest_pair[dim_x];
    c->position[dim_y] = closest_pair[dim_y];
...
void move_int(dungeon_t *d, character_t *c)
{
  ...
  if(c->position[dim_x] == c->last_known_pos[dim_x]
     && c->position[dim_y] == c->last_known_pos[dim_y]) {
    c->last_known_pos[dim_x] = -1;
    c->last_known_pos[dim_y] = -1;
  }

  if(c->last_known_pos[dim_x] < 0) {
    move_random(d,c);
  }
  ...
  //Order of my if statement:
  //1) Then it check if it is tunnelling type or if the new pair is moving to a place that is
  //not a wall. 
  //2) Then it checks the hardness of temp if less than the hardness of closest_pair.
  pair_t temp;
  for(int i = 0; i < 8; i++) {
    temp[dim_x] = c->position[dim_x] + p[i][0];
    temp[dim_y] = c->position[dim_y] + p[i][1];
...
void move_to_pc(dungeon_t *d, character_t *c)
{
  ...
  for(int i = 0; i < 8; i++) {
    temp[dim_x] = c->position[dim_x] + p[i][0];
    temp[dim_y] = c->position[dim_y] + p[i][1];
    if((abs(closest_pair[dim_x] - c->last_known_pos[dim_x]) >
	abs(temp[dim_x] - c->last_known_pos[dim_x]))
       && (abs(closest_pair[dim_y] - c->last_known_pos[dim_y]) >
	   abs(temp[dim_y] - c->last_known_pos[dim_y]))
       && (CHECK_TUN(a) || (mappair(temp) != ter_wall))
       && mappair(temp) != ter_wall_immutable){
	 closest_pair[dim_x] = temp[dim_x];
	 closest_pair[dim_y] = temp[dim_y];
    }
  }
  ...
  } else if((closest_pair[dim_x] == c->last_known_pos[dim_x])
	    && (closest_pair[dim_y] == c->last_known_pos[dim_y])){
    c->last_known_pos[dim_x] = -1;
    c->last_known_pos[dim_y] = -1;
  } else {
    c->position[dim_x] = closest_pair[dim_x];
...
{
  for(int i = 0; i < d->num_monsters + 1; i++) {
    if((p[dim_x] == d->characters[i].position[dim_x])
       && (p[dim_y] == d->characters[i].position[dim_y])
       && c->priority != d->characters[i].priority) {
      d->characters[i].is_alive = 0;
      d->characters[i].turn = INT_MAX;
      d->num_monsters--;
    }
  }
}
...
void check_int_tel_tun_err(dungeon_t *d)
{
  ...
  printf("%5s%5s%5s%5s\n","Err","Tun","Tel","Int");
  for(a = 0; a < 15; a++) {
    printf("%5c%5c%5c%5c %d\n", CHECK_ERR(a),CHECK_TUN(a),CHECK_TEL(a),CHECK_INT(a), a);
  }
}
