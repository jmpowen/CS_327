...
void io_handle_monster_lookup(dungeon *d)
{
  ...
  do {
    for(i = 0; i < 21; i++) {
      for(j = 0; j < 80; j++) {
	if (i == dest[dim_y] && j == dest[dim_x]) {
	  mvaddch(i+1,j,'*');
	} else if(d->character_map[i][j] != NULL) {
	  attron(COLOR_PAIR((color = d->character_map[i][j]->get_color())));
	  mvaddch(i+1,j,d->character_map[i][j]->symbol);
	  attroff(COLOR_PAIR(color));
	} else {
	  switch(d->map[i][j]) {
	  case ter_wall:
	...
	io_display_clear(d);
	mvprintw(4,0,"%s",d->character_map[dest[dim_y]][dest[dim_x]]->name);
	mvprintw(5,0,"%s",((npc *) d->character_map[dest[dim_y]][dest[dim_x]])->description);
	c = getch();
      }
    }
  } while (c != 27);
...
void io_handle_input(dungeon *d)
{
  ...
  do {
    ...
    case 'q':
      ...
      break;
    case ',':
      /* Picks up any item that PC is standing on and stores it in PC inventory, if it is a stack it removes the first option */
      if(d->objmap[d->PC->position[dim_y]][d->PC->position[dim_x]] != NULL && d->PC->inventory_size < MAX_INVENTORY) {
	pc_pick_up_item(d);
      } else {
	if(d->objmap[d->PC->position[dim_y]][d->PC->position[dim_x]] == NULL) {
	  mvprintw(0,0,"%s","No items at this position.");
	} else {
	  mvprintw(0,0,"%s","Not enough room in inventory.");
	}
      }
      fail_code = 1;
      break;
    case 'i':
      /* Shows items in the players inventory */
      io_handle_inventory(d);
      fail_code = 1;
      break;
    case 'e':
      io_handle_equipment(d);
      fail_code = 1;
      break;
    case 'x':
      /* Expunges item from the game */
      if(d->objmap[d->PC->position[dim_y]][d->PC->position[dim_x]] != NULL) {
	pc_expunge_item(d);
      } else {
	io_clear_msg_line();
	mvprintw(0,0,"%s","Nothing here.");
      }
      fail_code = 1;
      break;
    case 'L':
      /* This lets you target monsters and display their name/info*/
      io_handle_monster_lookup(d);
      fail_code = 1;
      break;
    default:
...
{
  int key, fail_code, select;
  fail_code = 1;
  select = 0;
  do {
    io_display_inventory(d, select);
    switch(key = getch()) {
    case '8':
    case 'k':
    case KEY_UP:
    ...
    case 'j':
    case KEY_DOWN:
      if(select < (int) d->PC->inventory_size -1)
	select++;
      fail_code = 1;
      break;
    case 'w':
      pc_equip_item(d,select);
      select = 0;
      fail_code = 1;
      break;
    case 'q':
    ...
    case 'd':
      //drop item
      pc_drop_item_inventory(d,select);
      select = 0;
      fail_code = 1;
      break;
    case 'I':
      io_display_item_info(d, select);
      fail_code = 1;
      key=getch();
      break;
    default:
...
void io_display_inventory(dungeon *d, int select)
{
  ...
  select += 6;
  io_display_clear(d);  
  mvprintw(4,25,"%s","PC inventory:");
  mvaddch(select-1,24, ' ');
  mvaddch(select,24,'*');
  mvaddch(select+1,24,' ');
  for(i = 0; i < 10; i++) {
    if(i < (int) d->PC->inventory_size)
      mvprintw(i+6, 25, "%d) %s", i, d->PC->inventory[i]->get_name());
  }
  mvprintw(20,0,"%s","w wear item | d drop item | I item information");
}

void io_display_item_info(dungeon *d, int select)
{
  io_display_clear(d);
  mvprintw(4,0,"%s", d->PC->inventory[select]->get_name());
  mvprintw(5,0,"%s", d->PC->inventory[select]->get_description());
}
...
void io_handle_equipment(dungeon *d)
{
  ...
  do {
    ...
    case 'j':
    case KEY_DOWN:
      if(select < MAX_EQUIPMENT-1)
	select++;
      fail_code = 1;
      break;
    case 't':
      if(d->PC->equipment[select] != NULL)
	pc_unequip_item(d,select);
      fail_code = 1;
      break;
    case 'q':
    case 'Q':
    ...
    case 'd':
      if(d->PC->equipment[select] != NULL)
	pc_drop_item_equipment(d,select);
      fail_code = 1;
      break;
    default:
      fail_code = 1;
...
void io_display_equipment(dungeon *d, int select)
{
  ...
  select += 6;
  io_display_clear(d);  
  mvprintw(4,25,"%s","PC equipment:");
  mvaddch(select-1,24, ' ');
  mvaddch(select,24,'*');
  mvaddch(select+1,24,' ');
  for(i = 0; i < MAX_EQUIPMENT; i++) {
    if(d->PC->equipment[i] != NULL) {
      mvprintw(i+6, 25, "%c) %s", equipment_order[i], d->PC->equipment[i]->get_name());
    } else {
      mvprintw(i+6, 25, "%c)", equipment_order[i]);
    }
  }
  mvprintw(20,0,"%s","d drop item | t take off an item");
}
