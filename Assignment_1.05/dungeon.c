...
static int make_stairs(dungeon_t *d) {
  uint16_t i, random_room, random_x, random_y;
  stair_t *s;
  
  d->stairs = malloc(sizeof (*d->stairs) * 2);
  for(i = 0; i < 2; i++) {
    random_room = random_x = random_y = 0;
    s = d->stairs + i;
    random_room = rand_range(0,d->num_rooms-1);
    random_x = rand_range(0,d->rooms[random_room].size[dim_x]-1);
    random_y = rand_range(0,d->rooms[random_room].size[dim_y]-1);

    random_x += d->rooms[random_room].position[dim_x];
    random_y += d->rooms[random_room].position[dim_y];

    if(i == 1
       && (d->stairs[0].position[dim_x] == (d->rooms[random_room].position[dim_x] + random_x))
       && (d->stairs[0].position[dim_y] == (d->rooms[random_room].position[dim_x] + random_y))) {
      i--;
      continue;
    } else {
      s->position[dim_x] = random_x;
      s->position[dim_y] = random_y;
    }
    if(i == 0 ) {
      s->symbol = '>';
    } else {
