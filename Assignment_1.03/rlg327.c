...
typedef uint8_t charPairs_t[num_dims];
...
int place_PC(dungeon_t *d) {
  ...
  random_place = rand() % d->num_rooms;
  p[dim_x] = rand() % d->rooms[random_place].size[dim_x]
    + d->rooms[random_place].position[dim_x];
  p[dim_y] = rand() % d->rooms[random_place].size[dim_y]
    + d->rooms[random_place].position[dim_y];

  d->pc[dim_x] = p[dim_x];
  d->pc[dim_y] = p[dim_y];
  d->cPC[dim_x] = (char) p[dim_x];
  d->cPC[dim_y] = (char) p[dim_y];
  d->map[d->pc[dim_y]][d->pc[dim_x]] = ter_player;
...
int write_dungeon(dungeon_t *d, char *file)
{
  ...
  fwrite(&d->cPC[dim_x], 1, 1, f);
  fwrite(&d->cPC[dim_y], 1, 1, f);
...
int read_dungeon(dungeon_t *d, char *file)
{
  ...
  fread(&d->cPC[dim_x], 1, 1, f);
  fread(&d->cPC[dim_y], 1, 1, f);

  d->pc[dim_x] = (int) d->cPC[dim_x];
  d->pc[dim_y] = (int) d->cPC[dim_y];
...
void dijkstra_corridor_path_finder(dungeon_t *d, pair_t pc) {  
  static corridor_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  heap_t h;
  uint32_t x, y;
  int initialized = 0;
...
void dijkstra_map_path_finder(dungeon_t *d, pair_t pc) {
  static corridor_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  heap_t h;
  uint32_t x, y;
  int initialized = 0;
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost >
         p->cost + 1 + (((int) d->hardness[p->pos[dim_y]][p->pos[dim_x]]) / 85))) {
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost =
        p->cost + 1 + (((int) d->hardness[p->pos[dim_y]][p->pos[dim_x]]) / 85);
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_x] = p->pos[dim_x];
...
int main(int argc, char *argv[])
{
 ...
 if (argc > 1) {
  ...
  dijkstra_corridor_path_finder(&d, d.pc);
  dijkstra_map_path_finder(&d, d.pc);
